...
export const batchUpdateProductUploadFile = async (req, res, next) => {
  const fileAction = await FileAction.create({
    fileType: FILE_TYPE.PRODUCT_UPDATE,
...
const batchUpdateProducts = async (squareName, squareId) => {
  ...
  const productUpdateData = await csv().fromFile(
    './uploads/Import-1696495930645.csv'
  )
  ...
  productLoop: for (let i = 0; i < organizedProducts.length; i++) {
    ...
    if (organizedProducts[i][0].productId) {
      console.log(organizedProducts[i][0].productId)
      //this is an existing product
      let currProductResponse, responseError
      try {
        currProductResponse =
      ...
      for (let j = 0; j < organizedProducts[i].length; j++) {
        ...
        //console.log('product:', currProductObject.object)
        ...
        //console.log('variations:', currProductVariations)
      }
      ...
      let newProductVariations = organizedProducts[i].map(
        (variation, index) => ({
          ...
          itemVariationData: {
            ...
            pricingType: 'FIXED_PRICING',
            priceMoney: {
              amount: parseInt(variation.variationPrice * 100) || 0,
              currency: 'CAD',
            },
            trackInventory: true,
            availableForBooking: false,
        ...
        try {
          ...
          for (let j = 0; j < organizedProducts[i].length; j++) {
            resultsJSON.push({
              ...organizedProducts[i][j],
              status: 'OK',
              message: 'Error initializing inventory for new products',
            })
          }
          console.log(error)
          continue productLoop
