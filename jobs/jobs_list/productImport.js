...
// your code goes here
export const productImport = async (jobAttrs) => {
  const {
    squareName,
    squareId,
    skuId,
  ...
  //processedIds is either a productID or a product name
  let processedIds = [''],
    organizedProducts = []
  let hasError = false
  ...
  for (let i = 0; i < productUpdateData.length; i++) {
    const currProductName = productUpdateData[i].productName.trim()

    if (
      productUpdateData[i]?.productId &&
      productUpdateData[i].productId != ''
    ) {
      const currProductId = productUpdateData[i].productId.trim()
      //this is an existing product
      if (!processedIds.includes(currProductId)) {
        processedIds.push(currProductId)
        let currProduct = productUpdateData.filter((row) => {
          return currProductId == row.productId
        })
        for (let j = 0; j < currProduct.length; j++) {
          Object.keys(currProduct[j]).forEach(
            (k) =>
              (currProduct[j][k] =
                typeof currProduct[j][k] == 'string'
                  ? currProduct[j][k].trim()
                  : currProduct[j][k])
          )
        }
    ...
    if (organizedProducts[i][0]?.productId) {
      ...
      try {
        currProductResponse =
          await squareClient.catalogApi.retrieveCatalogObject(
            organizedProducts[i][0].productId.trim(),
            false
          )
      } catch (error) {
        hasError = true
      ...
      if (organizedProducts[i][0].productName != '') {
        currProductObject.object.itemData.name =
          organizedProducts[i][0].productName.trim()
      }

      for (let j = 0; j < organizedProducts[i].length; j++) {
        let price = parseInt(
          organizedProducts[i][j].variationPrice.trim() * 100
        )
        ...
        let variationIndex = currProductVariations.findIndex((variation) => {
          return variation.id == organizedProducts[i][j].variationId.trim()
        })

        if (variationIndex >= 0) {
          console.log(currProductVariations[variationIndex])
          //variation ID exists, edit the existing one
          currProductVariations[variationIndex].itemVariationData.name =
            organizedProducts[i][j].variationName.trim()
          currProductVariations[variationIndex].itemVariationData.sku =
            organizedProducts[i][j].variationSku.trim()
          ...
          if (organizedProducts[i][j].variationSku) {
            variationSku = organizedProducts[i][j].variationSku
              .trim()
              .substring(0, 25)
          } else {
          ...
          if (vendorLocations.length > 0) {
            newVariation.presentAtLocationIds = vendorLocations
      ...
      if (vendorLocations.length > 0) {
        for (let i = 0; i < newProductVariations.length; i++) {
          newProductVariations[i].presentAtLocationIds = vendorLocations
          if (defaultInventoryWarningLevel > 0) {
      ...
      if (vendorLocations.length > 0) {
        newProductObject.presentAtLocationIds = vendorLocations
      }
